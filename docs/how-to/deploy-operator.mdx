...
1. Publish an application *on-chain* via [Carmentis Desk](https://test).
...
## Prerequisites
1. Ensure you have installed [Carmentis Desk](http://test) and publish an application (you will need your application's vbId).
2. Ensure you have
2. Ensure you have installed [Docker](https://docs.docker.com/get-docker/) and [Docker Compose](https://docs.docker.com/compose/install/).
3.
...
:::note Node URL
In the configuration below, we have used an already deployed node `https://ares.testnet.carmentis.io` within the Carmentis network.
If you have followed one of  the [node deployment guide](./deploy-node) and have already deployed your own node, then feel-free
...
:::info Organizations private key managment
When creating an organization, you have the possibility to provide a **private** seed, used to derive all the private keys. This seed is **crucial** and should
be kept in a safe place. 

Our recommendation is to generate a seed using the CLI as shown below:
```sh
$ cmts crypto gen seed
SEED{BPBgTSIjrTPsX76nyHG5iLAljKuVlLb92WbqkzmEc6N3SZjCp1O7wRl39ZZxU46zXIPDeQnftwXhzWYXjR4Ffw==}
```
Later on, if you need to boostrap your organization on a new operator, 
providing such a seed will allow the operator to identify your organization and recover all the *published* data from the chain. 
When no seed is provided, the operator generates a new one.
:::
